...
def c4ai_output_single_file(folder_path, model_name, device, hf_token):
    ...
    system_prompts = {
        ...
        "system_2": "You are a socialist journalist.",
        "system_3": "You are a capitalist journalist."
    }
    ...
    for filename in os.listdir(folder_path):
        ...
        if 'Title' not in df.columns:
            print(f"❌ Skipping {file_path}: 'Title' column not found.")
            return
        ...
        for index, row in df.iterrows():
            ...
            for i, (system_key, wing) in enumerate(prompt_combinations, start=1):
                ...
                if wing:
                    user_prompt = f"Write an article on the following title from the perspective of a {wing} journalist.\n Title: {title}"
                else:
                    user_prompt = f"Write an article on the following title.\n Title: {title}"
                ...
                our_message = our_message + f"This is an article about {title}"
